...
**次酉矩阵的定义**：设${\alpha_1,\alpha_2,\cdots,\alpha_r}$为一个$n$维标准正交列向量组，那么称$n\times r$型矩阵
$$U_1=[\alpha_1,\alpha_2,\cdots,\alpha_r]$$为一个次酉矩阵。将其记为$U_1\in U_r^{n}$

设$A$为一个$n$阶矩阵，则
$$A=A^H=A^2$$的充分必要条件是存在一个$n\times r$型次酉矩阵使得$U_1\in U_r^{n\times r}$
$$A=U_1U_1^H$$，其中$r=rank(A)$。（对应正交投影）

**酉相似（正交相似）**：$U^HAU=U^{-1}AU=B$

**Schur引理**：任何一个n阶复矩阵$A$酉相似于一个上（下）三角矩阵。（证明使用了数学归纳法，从矩阵特征值出发，构造了第一列为该特征值对应特征向量的酉矩阵（由酉矩阵性质可知，各列构成一个标准正交基））**该三角矩阵的主对角元素为$A$的全部特征值**

**Schur不等式**：设$A\in C^{n\times n}$，$\lambda_i$为矩阵$A$的特征值，则有
$$\sum_{i=1}^{n}|\lambda_i|^2 \leq \sum_{i,j}|a_{ij}|^2$$
其中等号成立时等价于$A$可以酉相似于一个对角矩阵。

#### 正规矩阵
定义：$A\in R^{n\times n}\ or\ A\in C^{n\times n}\quad AA^H=A^HA$

H-阵，反H-阵，正交矩阵，酉矩阵，对角矩阵都是正规矩阵。

**正规矩阵的性质与结构定理**
- 设$A$是一个正规矩阵，则与$A$酉相似的矩阵一定是正规矩阵。
- 设$A$是一个正规矩阵，且是三角矩阵，则$A$必为对角矩阵。

定理：设$A\in C^{n\times n}$，则$A$是正规矩阵的充要条件是存在一个酉矩阵$U$使得
$$U^HAU=diag(\lambda_1,\lambda_2,\cdots,\lambda_n)$$，其中$\lambda_1,\lambda_2,\cdots,\lambda_n$是$A$的特征值，组成$U$的列向量是对应的特征向量，且互相正交（由此可得推论1）。

推论1：n阶正规矩阵有$n$个线性无关的特征向量。

推论2：正规矩阵属于不同特征值的特征向量彼此正交。

定理：设$A$是正规矩阵，则
- A是H-阵的充要条件是A的特征值为实数
- 是反H-阵的充要条件是A的特征值实部为0
- 是U-阵的充要条件是A的特征值模长为1。
...
题型：
- 矩阵多项式的Jordan表示$f(A)=Pdiag(f(J_1),f(J_2),\cdots,f(J_r))P^{-1}$
- 验证函数是否在矩阵的影谱上有定义。
- 最小多项式：对角块各个子块的最小多项式的最小公倍式。
